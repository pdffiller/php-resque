...
class Resque_Worker
{
        ...
        while (true) {
            ...
            if (!$job) {
                ...
                if ($blocking === false) {
                    ...
                    $this->logger->log(Psr\Log\LogLevel::INFO, 'Sleeping for {interval}',
                        array('interval' => $interval));
                    if ($this->paused) {
    ...
    public function perform(Resque_Job $job)
    {
        try {
            ...
            $this->logger->log(Psr\Log\LogLevel::CRITICAL, '{job} has failed {stack}',
                array('job' => $job, 'stack' => $e));
            $job->fail($e);
            return;
        }
    ...
    public function killChild()
    {
        ...
        if (exec('ps -o pid,state -p ' . $this->child, $output, $returnCode) && $returnCode != 1) {
            $this->logger->log(Psr\Log\LogLevel::DEBUG, 'Child {child} found, killing.',
